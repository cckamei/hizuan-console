  ...
  <div id="home_banner">
    <div class="search-box search-tab">
        ...
        <el-row :gutter="25">
          <el-col :span="6">
            <div class="search-main">
              <span class="search-text">轮播名称</span>
              <el-input placeholder="按轮播名称搜索" class="search-input" v-model="search.name"></el-input>
            </div>
          </el-col>

          <el-col :span="6">
            <div class="search-main"><span class="search-text">轮播类型</span>
          ...
          <el-col :span="6">
            <div class="search-main">
              <span class="search-text">轮播内容</span>
              <el-input placeholder="按轮播内容搜索" class="search-input" v-model="search.content"></el-input>
            </div>
          </el-col>
      ...
      <div class="search-btn">
        <el-button cricle type="primary" @click="get_home_banners()">查询</el-button>
        <el-button cricle type="primary" @click="add_line">新增首页轮播</el-button>
      </div>
    </div>
    <div class="search-table search-tab">
      <el-table :data="tableData" stripe align="center">
        ...
        <el-table-column label="轮播内容" align="center" prop="content.name">
        </el-table-column>
        <el-table-column label="轮播封面" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" align="center" fixed="right">
          <template slot-scope="scope">
            <el-button v-if="scope.row.active" @click.native.prevent="inactive_home_banner(scope.$index, tableData)" type="text" size="small">当前启用</el-button>
            <el-button v-else @click.native.prevent="active_home_banner(scope.$index, tableData)" type="text" size="small">当前禁用</el-button>
            <el-button @click.native.prevent="edit_home_banner(scope.$index, tableData)" type="text" size="small">编辑</el-button>
            <el-button @click.native.prevent="delete_home_banner(scope.$index, tableData)" type="text" size="small">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="编辑首页轮播" :visible.sync="dialogShow" width="30%">
      <el-form label-width="120px" :model="added">
        ...
        <el-form-item label="轮播内容" v-show="added.type == 1">
        ...
        <el-form-item label="轮播内容" v-show="added.type == 2" prop="content_id">
          <el-select v-model="added['content_id']" class="added-input" @change="selectChange">
        ...
        <el-form-item label="轮播封面">
          <el-upload drag :action="upload_url" :on-success="handleUploadSuccess">
            ...
            <img width="60%" height="auto" :src="added.image" alt="">
          </el-upload>
        </el-form-item>
      </el-form>
...
export default {
  ...
  mounted() {
    ...
    this.get_home_banners();
  },
  methods: {
    selectChange(value) {
      console.log(value);
      Vue.set(this.added, "content_id", value);
    },
    typeChange(value) {
      this.added.content_id = "";
    },
    async active_home_banner(index, rows) {
      let bid = rows[index].id;
      let result = await this.$axios.post(`/admin/home_banner/${bid}`, {
        active: true
      });
    ...
    async inactive_home_banner(index, rows) {
      let bid = rows[index].id;
      let result = await this.$axios.post(`/admin/home_banner/${bid}`, {
        active: false
      });
    ...
    async sureAdd() {
      let home_bannerId = this.added.id;
      let home_banner = this.added;
      if (this.added.id) {
        let result = await this.$axios.post(
          `/admin/home_banner/${home_bannerId}`,
          home_banner
        );
        if (result.data.status != 0) return;
        ...
        let result = await this.$axios.post("/admin/home_banner", home_banner);
        if (result.data.status != 0) return;
        this.added = JSON.parse(JSON.stringify(added_init));
        this.$message("新增成功");
        this.dialogShow = false;
    ...
    async edit_home_banner(index, row) {
      let home_banner = row[index];
      let home_banner_id = home_banner.id;
      this.added = JSON.parse(JSON.stringify(home_banner));
    ...
    async delete_home_banner(index, rows) {
      ...
      let home_banner_id = rows[index].id;
      if (home_banner_id) {
        let resp = await this.$axios.delete(
          `/admin/home_banner/${home_banner_id}/`
        );
        rows.splice(index, 1);
      } else {
        rows.splice(index, 1);
    ...
    async get_home_banners() {
      let params = this.search;
      let resp = await this.$axios.get("/admin/home_banner/", {
        params: params
      });
