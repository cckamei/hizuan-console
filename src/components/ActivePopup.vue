<template>
  <div class="ActivePopup">
    ...
    <div style="display: block; float: left;">
        <span>活动弹窗</span>
        <el-button cricle type="primary" @click="add_activePopup">新增</el-button>
    </div>
    <br> <br>
    <el-table :data="tableData" border>
      ...
      <!-- <el-table-column label="弹窗点击行为" width="200px" align="center">
        <template slot-scope="scope">
    ...
    export default {
        ...
        data (){
            return {
                ...
                // activeType: [{activate_banner_id: 0,title: '充值大优惠'},{activate_banner_id: 1,title: '热门促销'}],
                activeType: [],
                isShowDialog: false,
                isMessageShow: false,
        ...
        mounted() {
            this.get_activePopup();
        },
        methods: {
            async get_activePopup (){
                let res = await this.$axios.get('/admin/pop_banner/');
                this.activeType = res.data.data.list_activate;
                this.tableData = res.data.data.list;
                this.total = res.data.data.total;
                console.log(res.data,this.activeType);
            },
            add_activePopup: function (){  //新增
            ...
            async save_active (index, row){ //保存
                ...
                if (active_id) {
                    console.log("正在修改");
                    let result = await this.$axios.post(`/admin/pop_banner/${active_id}/`, { //修改后保存
                    ...
                    if (result.data.code == 0) {
                        this.$message("修改成功");
                        this.get_activePopup(); //修改成功后初始化页面一次
                    }
                }else {
                    // 新增
                    let result = await this.$axios.post("/admin/pop_banner/", {
                        active: active.active,
                        img: active.img,
                        command: active.command,
                        sort_id: active.sort_id,
            ...
            async delete_active(index, rows) {
                ...
                if (active_id) {
                    let res = await this.$axios.delete(`/admin/pop_banner/${active_id}/`);
                    rows.splice(index, 1);
                } else {
                    rows.splice(index, 1);
                }
            ...
            async handleCurrentChange(val) {
                ...
                let resp = await this.$axios.get("/admin/pop_banner/?" + param);
